...
export default function ViewSchedule({
	...
	const initialDate = _initialDate
		? toZonedTime(_initialDate, "Asia/Tokyo")
		: undefined;
	...
	useEffect(() => {
		if (!date) setAttendance([]);
			...
			startTransition(async () => {
				...
				setAttendance(
					await Promise.all(
						schedule.map(async ({ id, date, start, end, description }) => {
							const { present, absent, late, undecided } =
								await overallAttendanceDetail(id);
				...
				isPending ? (
					...
					<div className="w-full flex flex-col divide-y text-slate-500">
						<div className="flex items-center justify-between">
							<div className="flex items-center gap-x-1 p-1 font-medium text-xl">
								<Calendar className="w-5 h-5 mx-1" />
								<span>
									{formatInTimeZone(date, "Asia/Tokyo", "MM/dd (eee)", {
										locale: ja,
									})}
								</span>
							</div>
						...
						{attendace.map(
							(
								{ start, end, description, present, absent, late, undecided },
								idx,
							) => (
								<div key={idx} className="flex flex-col gap-y-2 p-2">
									{(start || end) && (
									...
									{present.length > 0 && (
										<div className="flex items-stretch gap-x-2">
											<div className="[writing-mode:vertical-rl] flex justify-center rounded bg-green-500 text-white">
												出席
											</div>
									...
									{absent.length > 0 && (
										<div className="flex items-stretch gap-x-2">
											<div className="[writing-mode:vertical-rl] flex justify-center rounded bg-red-500 text-white">
												欠席
											</div>
											<div className="flex flex-wrap gap-2">
									...
									{late.length > 0 && (
										<div className="flex items-stretch gap-x-2">
											<div className="[writing-mode:vertical-rl] flex justify-center rounded bg-yellow-500 text-white">
												遅刻
											</div>
											<div className="flex flex-wrap gap-2">
									...
									{undecided.length > 0 && (
										<div className="flex items-stretch gap-x-2">
											<div className="[writing-mode:vertical-rl] flex justify-center rounded bg-gray-500 text-white">
												未定
											</div>
											<div className="flex flex-wrap gap-2">
						...
						>
							...
							{(start || end) && (
								<div className="flex items-center gap-x-1">
									...
									{(start || end) && <span>-</span>}
									{end && (
										<span>{formatInTimeZone(end, "Asia/Tokyo", "HH:mm")}</span>
									)}
								</div>
							)}
							{description && (
...
export function NewSchedule() {
	...
	const handleDescription = (_idx: number, description: string) => {
		setRows(
			rows.map((row, idx) => (idx === _idx ? { ...row, description } : row)),
		);
	};
			...
			<Table>
				...
				<TableBody>
					{rows.map((row, idx) => (
						<TableRow key={idx}>
							<TableCell>
								<ListMinus onClick={() => deleteRow(idx)} className="w-16" />
							</TableCell>
							<TableCell>
								<input
									type="date"
...
export function EditSchedule() {
	...
	useEffect(() => {
		const fetchSchedule = async () => {
			...
			setRows(
				schedule.map(({ id, date, start, end, description }) => {
					const jstDate = formatInTimeZone(date, "Asia/Tokyo", "yyyy-MM-dd");

					const jstStart = start
						? formatInTimeZone(start, "Asia/Tokyo", "HH:mm")
						: "";
	...
	const deleteRow = (_idx: number) => {
		setRows(
			rows.map((row, idx) =>
				idx === _idx ? { ...row, toDelete: !row.toDelete } : row,
			),
		);
	};
	...
	const handleDate = (_idx: number, date: string) => {
		setRows(
			rows.map((row, idx) =>
				idx === _idx ? { ...row, date, isEdited: true } : row,
			),
		);
	};
	...
	const handleStart = (_idx: number, start: string) => {
		setRows(
			rows.map((row, idx) =>
				idx === _idx ? { ...row, start, isEdited: true } : row,
			),
		);
	};
	...
	const handleEnd = (_idx: number, end: string) => {
		setRows(
			rows.map((row, idx) =>
				idx === _idx ? { ...row, end, isEdited: true } : row,
			),
		);
	};
	...
	const handleDescription = (_idx: number, description: string) => {
		setRows(
			rows.map((row, idx) =>
				idx === _idx ? { ...row, description, isEdited: true } : row,
			),
		);
	};
			...
			<Table>
				...
				<TableBody>
					{rows.map((row, idx) => (
						...
						>
							<TableCell>
								<div onClick={() => deleteRow(idx)}>
									{row.toDelete ? (
										<ListX className="w-16 text-red-500" />
									) : (
										<ListMinus className="w-16" />
									)}
								</div>
							</TableCell>
							<TableCell>
