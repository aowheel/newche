...
export function ViewGuide() {
	return (
		<div className="prose p-4 bg-slate-100 rounded-xl">
			<h3>使い方ガイド</h3>
			<ul>
				<li>
					表示されているのは今日以降の日程で「出席」または「遅刻」と答えた人の一覧です。
				</li>
				<li>
					カレンダーの中の
				...
				<li>
					もとの表示に戻す場合は
					<Undo2 className="w-4 mx-1 inline" />
					を押してください。
				</li>
			</ul>
		</div>
	...
	return (
		<div className="prose p-4 bg-slate-100 rounded-xl">
			...
			<ul>
				...
				<li>
					<span className="rounded-full mx-1 px-2 py-1 text-sm bg-slate-900 text-white">
						出欠席
					</span>
...
export function ManageGuide() {
	return (
		<div className="prose p-4 bg-slate-100 rounded-xl">
			...
			<ul>
				<li>
					<Plus className="w-4 mx-1 inline" />
					ボタンで行を追加します。
				</li>
				<li>日付が必須、開始時間、終了時間、詳細がオプションです。</li>
				<li>
					<ListMinus className="w-4 mx-1 inline" />
					ボタンで行を削除できます。
				</li>
			...
			<ul>
				<li>
					<ListMinus className="w-4 mx-1 inline" />
					ボタンで削除する行にチェックします。
					<ListX className="w-4 mx-1 inline text-red-500" />
					が送信時に削除される行です。
				</li>
				<li>変更時も、必須、オプションの項目は新規の場合と同様です。</li>
				<li>
					送信ボタンを押してデータを送信します。変更のない行は送信されません。
				</li>
...
export function FriendGuide({
	...
	return (
		<div className="prose m-4 p-4 bg-slate-100 rounded-xl">
			<h2>別のグループでのNewcheの利用</h2>
			<p>
				Newcheを友だち追加して別のグループトークに招待することで、データを共有しつつNewcheの機能を利用することができます。
			</p>
			<strong>
				無関係なグループトークには招待しないでください。不要になった場合はグループからBotを削除してください。
			</strong>
			<h3>追加済みグループ一覧</h3>
			...
			<p>
				新規・編集、前日20時の通知 / 上限 :{" "}
				<strong className="inline-block">{status}</strong>
			</p>
		</div>
	);
}
