...
export async function memberJoined(replyToken: string, members: any[]) {
	let text = "";
...
export async function notifyCreatedSchedule(schedule: Schedule[]) {
	...
	schedule.forEach(({ date, start, end, description }, idx) => {
		const _date = formatInTimeZone(date, "Asia/Tokyo", "MM/dd (eee)", {
			locale: ja,
		});
		const _start = start
...
export async function notifyNextWeekSchedule(replyToken?: string) {
	...
	const count = await Promise.all(
		schedule.map(({ id }) => countPresentOrLate(id)),
	);
